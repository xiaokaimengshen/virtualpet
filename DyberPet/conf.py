...
class PetConfig:
    ...
    def init_config(cls, pet_name: str, pic_dict: dict):
        ...
        with open(path, 'r', encoding='utf-8-sig') as f:
            ...
            act_conf = dict(json.load(open(act_path, 'r', encoding='utf-8-sig')))
            act_dict = {}
            ...
            if os.path.isfile(msg_file):
                msg_data = dict(json.load(open(msg_file, 'r', encoding='utf-8-sig')))
...
def CheckCharFiles(folder):
    ...
    try:
        ...
        pet_conf = json.load(open(path, 'r', encoding='utf-8-sig'))
    except:
        return 1, None

    try:
        path = os.path.join(folder, 'act_conf.json')
        act_conf = json.load(open(path, 'r', encoding='utf-8-sig'))
    except:
        return 2, None
...
class ActData:
    ...
    def init_config(self):
        if os.path.isfile(self.file_path):
            ...
            try:
                allAct_params = json.load(open(self.file_path, 'r', encoding='utf-8-sig'))
                self.fileGood = True
            except:
                #File broken
                allAct_params = {}
    ...
    def _check_actlist(self, petname, act_params, fv_lvl):
        ...
        pet_conf = json.load(open(pet_conf_file, 'r', encoding='utf-8-sig'))
        all_names_in_conf = []
...
class PetData:
    ...
    def init_data(self):
        ...
        if os.path.isfile(self.file_path):
            # Check file integrity
            try:
                allData_params = json.load(open(self.file_path, 'r', encoding='utf-8-sig'))
                self.saveGood = True
            except:
    ...
    def reset_all_progress(self, full_hp_inner=200):
        """
        Reset all pet save data to a fresh start.
        """
        self.frozen_data = False
        now = datetime.now()
        today = '%i-%i-%i' % (now.year, now.month, now.day)
        full_hp_inner = int(full_hp_inner)
        if full_hp_inner <= 0:
            full_hp_inner = 200
        ...
        self.allData_params = new_data
        if self.current_pet not in self.allData_params:
            self.current_pet = self.petsList[0]
        curr = self.allData_params[self.current_pet]
        self.hp = curr['HP']
        self.hp_tier = curr['HP_tier']
        self.fv = curr['FV']
        self.fv_lvl = curr['FV_lvl']
        self.items = curr['items']
        self.coins = curr['coins']
        self.days = curr['days']
        self.last_opened = curr['last_opened']
        self.save_data()
...
class TaskData:
    ...
    def init_data(self):
        ...
        if os.path.isfile(self.file_path):
            # Check file integrity
            try:
                self.taskData = json.load(open(self.file_path, 'r', encoding='utf-8-sig'))
                self.stateGood = True
            except:
                #File broken (seen by a few users)
...
class DiaryData:
    """Per-pet diary storage with raw records + daily journal summaries."""

    def __init__(self):
        self.file_path = os.path.join(configdir, 'data/diary_data.json')
        self.max_entry_per_day = 200
        self.max_entry_chars = 240
        self.max_pages = 12
        self.page_chars = 230
    ...
    def _load_data(self):
        if os.path.isfile(self.file_path):
            try:
                data = json.load(open(self.file_path, 'r', encoding='utf-8-sig'))
    ...
    def _migrate_legacy_schema(self):
        """
        Legacy schema:
            { petname: { "YYYY-M-D": [entries...] } }
        New schema:
            { petname: { "entries": {...}, "journals": {...}, "meta": {...} } }
        """
        migrated = {}
        for petname, node in self.data.items():
            if not isinstance(node, dict):
                continue
            if 'entries' in node or 'journals' in node:
                entries = node.get('entries', {})
                journals = node.get('journals', {})
                meta = node.get('meta', {})
            else:
                # Legacy: daily entries directly under pet node.
                entries = {}
                for day_key, day_entries in node.items():
                    if isinstance(day_entries, list):
                        entries[day_key] = day_entries
                journals = {}
                meta = {}

            migrated[petname] = {
                'entries': entries if isinstance(entries, dict) else {},
                'journals': journals if isinstance(journals, dict) else {},
    ...
    def _format_day(self, dt):
        return f"{dt.year}-{dt.month}-{dt.day}"

    def _normalize_text(self, message):
        if message is None:
            return ""
    ...
    def _ensure_node(self, petname, day):
        if petname not in self.data or not isinstance(self.data[petname], dict):
            self.data[petname] = {'entries': {}, 'journals': {}, 'meta': {}}
        pet_node = self.data[petname]
        if 'entries' not in pet_node or not isinstance(pet_node['entries'], dict):
            pet_node['entries'] = {}
        if 'journals' not in pet_node or not isinstance(pet_node['journals'], dict):
            pet_node['journals'] = {}
        if 'meta' not in pet_node or not isinstance(pet_node['meta'], dict):
            pet_node['meta'] = {}
        if day not in pet_node['entries'] or not isinstance(pet_node['entries'][day], list):
            pet_node['entries'][day] = []

    def add_entry(self, petname, category, message):
        if not petname:
            return
        ...
        self.data[petname]['entries'][day].append(entry)

        if len(self.data[petname]['entries'][day]) > self.max_entry_per_day:
            self.data[petname]['entries'][day] = self.data[petname]['entries'][day][-self.max_entry_per_day:]

        self.save_data()

    def get_entries(self, petname, day=None, limit=50):
        if not petname:
            return []
        day = day or self._today()
        try:
            entries = self.data.get(petname, {}).get('entries', {}).get(day, [])
    ...
    def list_journal_days(self, petname):
        try:
            day_keys = list(self.data.get(petname, {}).get('journals', {}).keys())
        except:
            day_keys = []
        valids = []
        for d in day_keys:
            if self._parse_day(d):
                valids.append(d)
        return sorted(valids, key=lambda x: self._parse_day(x), reverse=True)

    def get_journal(self, petname, day):
        try:
            journal = self.data.get(petname, {}).get('journals', {}).get(day, None)
            if isinstance(journal, dict) and journal.get('content'):
                return journal
            return None
        except:
            return None
    ...
    def _split_pages(self, text):
        ...
        for ch in text:
            paragraph.append(ch)
            count += 1
            if count >= self.page_chars:
                pages.append(''.join(paragraph))
                paragraph = []
                count = 0
                if len(pages) >= self.max_pages:
                    break
        if paragraph and len(pages) < self.max_pages:
            pages.append(''.join(paragraph))
        return pages

    def _humanize_entry_text(self, category, text):
        ...
        # Normalize technical English logs into readable Chinese diary lines.
        if "patpat interaction" in lower:
            return "你轻轻摸了摸我。"
        if lower.startswith("triggered: random"):
            return ""
        if lower.startswith("fed "):
            item = raw[4:].strip()
            return f"你喂了我 {item}。"
        if lower == "opened chat window":
            return "你打开了聊天窗口。"
        ...
        # Keep user/pet chat content as-is.
        if cat in ('chat_user', 'chat_pet', 'chat_pet_auto'):
            return raw

        # If text is mostly ASCII technical tokens, skip it in highlights.
        ascii_ratio = sum(1 for ch in raw if ord(ch) < 128) / max(1, len(raw))
        if ascii_ratio > 0.8 and cat in ('interaction', 'random', 'system'):
            return ""

        return raw

    def _llm_story_summary(self, petname, day, first_time, last_time, total, mood, highlights, chats=None):
        """
        Generate a story-style Chinese diary summary via online LLM when available.
        ...
        llm = LocalLLMService()
        if llm.unavailable_reason():
            return ""

        snippets = "；".join(highlights[:10]) if highlights else "今天比较平稳。"
        chat_snippets = "；".join((chats or [])[:8]) if chats else "今天聊天不多。"
        prompt = (
            f"你是桌宠{petname}。请把 {day} 的经历写成一段中文日记故事。"
            f"要求：第一人称、自然口吻、120~220字、不要分点、不要标题、不要英文、不要代码。"
            "请把用户和桌宠的对话自然融入故事，不要生硬照抄。"
            f"参考信息：记录时段 {first_time} 到 {last_time}，共 {total} 条，心情 {mood}。"
            f"关键片段：{snippets}。"
            f"聊天记录片段：{chat_snippets}"
        )
        try:
            text = llm.chat(
                messages=[{'role': 'user', 'content': prompt}],
                system_prompt='你是桌宠日记写作助手，只输出一段中文叙事正文。',
                max_tokens=360,
                temperature=0.7,
                timeout=18,
            )
            ...
            if not text:
                return ""
            # Basic sanitation: collapse multi-line bullet-like output into a paragraph.
            text = text.replace("\r", "\n")
    ...
    def _build_daily_summary(self, petname, day, entries):
        # Keep output short and diary-like; avoid making empty-day pages.
        if not entries:
            return None
        ...
        for e in entries:
            cat = str(e.get('category', 'system'))
            txt = str(e.get('text', '')).strip()
            category_count[cat] = category_count.get(cat, 0) + 1
            nice_txt = self._humanize_entry_text(cat, txt)
            if cat == 'interaction' and ("摸" in nice_txt or "patpat interaction" in txt.lower()):
                pat_count += 1
            if cat == 'feed':
                feed_count += 1
            if cat in ('chat_user', 'chat_pet', 'chat_pet_auto'):
                chat_count += 1
                if txt:
                    speaker = "你" if cat == 'chat_user' else petname
                    chat_dialogs.append(f"{speaker}：{txt}")
            if nice_txt and cat in ('chat_user', 'chat_pet', 'chat_pet_auto', 'feed', 'interaction', 'status_hp', 'status_fv', 'user_context'):
                highlights.append(nice_txt)

        # De-duplicate repetitive lines and keep only meaningful snippets.
        deduped = []
        seen = set()
        for h in highlights:
            key = h.strip()
            ...
            seen.add(key)
            deduped.append(key)
        highlights = deduped[:6]

        mood = "平静"
        hp_related = category_count.get('status_hp', 0)
        fv_related = category_count.get('status_fv', 0)
        pat_related = category_count.get('interaction', 0)
        if hp_related > 0:
            mood = "有点在意肚子"
        if fv_related > 0 or pat_related >= 3:
            mood = "开心"

        llm_story = self._llm_story_summary(
            petname=petname,
            day=day,
        ...
        if llm_story:
            ...
            # Fallback: still keep a paragraph narrative (no bullets).
            if highlights:
                snippets = "，".join(highlights[:3])
            else:
                snippets = "今天没有特别突出的片段，但你一直在陪我。"
            content = (
                f"今天从 {first_time} 到 {last_time}，我记录了 {total} 条与你有关的事情。"
                f"我整体心情是{mood}，我们之间有摸摸 {pat_count} 次、喂食 {feed_count} 次、对话 {chat_count} 次。"
                f"让我印象最深的是：{snippets}"
            ).strip()
            if chat_dialogs:
                chat_line = "；".join(chat_dialogs[-2:])
                content = f"{content} 我还记得我们聊到：{chat_line}。"
    ...
    def finalize_pending(self, petname, now=None):
        """
        Generate journals for all past days that have raw entries but no journal yet.
        This supports:
        - midnight auto summarization while running
        - catch-up summarization after next startup if PC was off.
        """
        if not petname:
            return 0
        self._ensure_node(petname, self._today())
        now = now or datetime.now()
        today = self._format_day(now)
        entries_node = self.data.get(petname, {}).get('entries', {})
        journals_node = self.data.get(petname, {}).get('journals', {})
        if not isinstance(entries_node, dict) or not isinstance(journals_node, dict):
            return 0

        created = 0
        for day in sorted(entries_node.keys(), key=lambda x: self._parse_day(x) or now):
            dt = self._parse_day(day)
            if dt is None:
                continue
            if day == today:
                continue
            if day in journals_node:
                continue
            day_entries = entries_node.get(day, [])
            if not isinstance(day_entries, list) or len(day_entries) == 0:
                # No real record, do not generate empty diary page.
                continue

            journal = self._build_daily_summary(petname, day, day_entries)
            if not journal:
                continue
            journals_node[day] = journal
            created += 1
    ...
    def generate_today_journal(self, petname, overwrite=True):
        """
        Generate (or regenerate) today's journal from today's raw entries.
        Returns 1 if created/updated, 0 otherwise.
        """
        if not petname:
            return 0
        day = self._today()
        ...
        if not isinstance(day_entries, list) or len(day_entries) == 0:
            return 0
        if (not overwrite) and (day in journals_node):
            return 0
...
class ItemData:
    ...
    def __init__(self, HUNGERSTR='Satiety', FAVORSTR='Favorability'):
        ...
        self.item_dict = {}
        #self.item_conf = dict(json.load(open(self.file_path, 'r', encoding='utf-8-sig')))
        self.reward_dict = {}
    ...
    def init_data(self):
        ...
        # Load subpets disabled: hide mini-pet content list.
        petItems = []
        ...
        for i, itemFolder in enumerate(sorted_itemMods):
            ...
            if os.path.exists(info_file):
                info = dict(json.load(open(info_file, 'r', encoding='utf-8-sig')))
            ...
            item_conf = dict(json.load(open(conf_file, 'r', encoding='utf-8-sig')))
...
def load_ItemMod(configPath, HUNGERSTR='Satiety', FAVORSTR='Favorability'):
    ...
    item_conf = dict(json.load(open(configPath, 'r', encoding='utf-8-sig')))
...
def checkItemMOD(itemFolder):
    ...
    try:
        item_dict = dict(json.load(open(configFile, 'r', encoding='utf-8-sig')))
    except:
        return 1, None
