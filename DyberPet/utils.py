...
def read_json(conf_file):
    ...
    :return: map
    """
    with open(conf_file, 'r', encoding='utf-8-sig') as file:
...
def get_active_window_brief():
    """
    Lightweight foreground window sampler (Windows only).
    Returns:
        dict(app=<exe>, title=<window_title>, pid=<int>) or None
    """
    ...
    try:
        ...
        if hproc:
            try:
                ...
                ok = kernel32.QueryFullProcessImageNameW(
                    hproc, 0, exe_buf, ctypes.byref(size)
                )
                if ok:
        ...
        return {"app": app_name, "title": title, "pid": pid_val}
    except Exception:
        return None
