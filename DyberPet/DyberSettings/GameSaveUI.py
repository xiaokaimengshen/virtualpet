...
class SaveInterface(ScrollArea):
    ...
    def _loadin_petData(self, folder, petname):
        ...
        save_dict = json.load(open(os.path.join(folder, 'pet_data.json'), 'r', encoding='utf-8-sig'))
    ...
    def __onCardSaveClicked(self, cardIndex):
        ...
        # Record info file
        infoFile = open(os.path.join(finalFolder, 'info.txt'), 'w', encoding='utf-8-sig')
    ...
    def __onCardLoadinClicked(self, cardIndex):
        ...
        info = open(os.path.join(jsonPath,'info.txt'),'r', encoding='utf-8-sig').readlines()
...
def check_info_file(file):
    info = open(file, 'r', encoding='utf-8-sig').readlines()
...
def check_data_file(file, petname):
    try:
        allData_params = json.load(open(file, 'r', encoding='utf-8-sig'))
