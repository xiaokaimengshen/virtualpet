...
class QuickSaveCard(SimpleCardWidget):
    ...
    def __init_SaveCard(self):
        ...
        # Load in info
        info = open(os.path.join(self.jsonPath,'info.txt'),'r', encoding='utf-8-sig').readlines()
        ...
        if os.path.exists(infoJson):
            infoJson = json.load(open(infoJson, 'r', encoding='utf-8-sig'))
        ...
        '''
        saveData = json.load(open(os.path.join(self.jsonPath,'pet_data.json'), 'r', encoding='utf-8-sig'))
...
class CharLine(SimpleCardWidget):
    ...
    def __init_InfoList(self):
        ...
        if not os.path.exists(infoFile):
            ...
            infoConfig = json.load(open(infoFile, 'r', encoding='utf-8-sig'))
...
class CharCardWidget(SimpleCardWidget):
    ...
    def __init__(self, cardIndex: int, jsonPath=None, petFolder=None, parentDir='role', parent=None):
        ...
        self.setFixedSize(INFOCARD_W+10, INFOCARD_H + 150)
    ...
    def __init_InfoCard(self):
        ...
        infoConfig = json.load(open(self.jsonPath, 'r', encoding='utf-8-sig'))
        ...
        self._initDiaryWidget()
        self.vBoxLayout2.addWidget(HorizontalSeparator(QColor(20,20,20,60), 1))
        self.vBoxLayout2.addWidget(self.diaryTitle, 0, Qt.AlignLeft)
        self.vBoxLayout2.addWidget(self.diaryScroll, 0, Qt.AlignLeft)
        self.vBoxLayout2.addStretch(1)
        

        # Author Info (branding override)
    ...
    def _initDiaryWidget(self):
        self.diaryTitle = CaptionLabel(self.tr("Diary (Today)"))
        setFont(self.diaryTitle, 13, QFont.DemiBold)
        self.diaryTitle.adjustSize()

        self.diaryScroll = ScrollArea(self)
        self.diaryScroll.setFixedSize(300, 110)
        self.diaryScroll.setWidgetResizable(True)
        self.diaryScroll.setStyleSheet("""QScrollArea { background-color: transparent; border: none; }""")

        self.diaryWidget = QWidget(self.diaryScroll)
        self.diaryLayout = QVBoxLayout(self.diaryWidget)
        self.diaryLayout.setContentsMargins(0, 0, 0, 0)
        self.diaryLayout.setSpacing(0)

        self.diaryLabel = CaptionLabel("", self.diaryWidget)
        self.diaryLabel.setWordWrap(True)
        setFont(self.diaryLabel, 12)
        self.diaryLayout.addWidget(self.diaryLabel, 0, Qt.AlignLeft | Qt.AlignTop)
        self.diaryLayout.addStretch(1)
        self.diaryScroll.setWidget(self.diaryWidget)
        self.refreshDiary()

    def refreshDiary(self):
        if not hasattr(settings, 'diary_data'):
            self.diaryLabel.setText(self.tr("Diary is not initialized yet."))
            return

        today = datetime.now()
        day = f"{today.year}-{today.month}-{today.day}"
        entries = settings.diary_data.get_entries(self.petFolder, day=day, limit=20)
        if not entries:
            self.diaryLabel.setText(self.tr("No diary entry for today yet."))
            return
        ...
        for entry in entries:
            ...
            msg = entry.get('text', '')
            if msg:
                text_lines.append(f"[{t}] {msg}")
        if not text_lines:
            text_lines = [self.tr("No diary entry for today yet.")]
        self.diaryLabel.setText("\n".join(text_lines))
        self.diaryLabel.adjustSize()
...
class ItemCardWidget(SimpleCardWidget):
    ...
    def __init_InfoCard(self):
        ...
        infoConfig = json.load(open(infoJsonPath, 'r', encoding='utf-8-sig'))
...
def AvatarImage(image, edge_size=65, frameColor=QColor(255, 255, 255, 0)): #"#ffffff"):
    ...
    if image.isNull() or image.width() <= 0 or image.height() <= 0:
        placeholder = QImage(edge_size, edge_size, QImage.Format_ARGB32)
