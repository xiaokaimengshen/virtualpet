...
class BubbleManager(QObject):
    ...
    def load_bubble_config(self) -> dict:
        ...
        bubble_conf = dict(json.load(open(system_conf_file, 'r', encoding='utf-8-sig')))

        # Load any changes made in pet config
        if os.path.exists(pet_bb_conf_file):
            pet_bb_conf = dict(json.load(open(pet_bb_conf_file, 'r', encoding='utf-8-sig')))
    ...
    def trigger_bubble(self, bb_type):
        ...
        message = self._localize_message(message)
    ...
    def prepare_feed_required(self):
        ...
        bubble_dict['message'] = self._localize_message(bubble_dict['message'])
    ...
    def _localize_message(self, message: str) -> str:
        ...
        # Normalize common English templates to Chinese.
        replacements = [
            (r"(?i)\bI want to have\b", "想吃"),
            (r"(?i)\bYou should be focusing on your work\b", "要专心做事哦"),
            (r"(?i)\bGood Morning!?\b", "早上好！"),
            (r"(?i)\bGood Afternoon!?\b", "下午好！"),
            (r"(?i)\bGood Evening!?\b", "晚上好！"),
            (r"(?i)\bTime to sleep!?\b", "该睡觉啦！"),
        ]
        for pattern, repl in replacements:
            txt = re.sub(pattern, repl, txt)

        # If English still remains, strip ASCII words so bubble never shows English.
