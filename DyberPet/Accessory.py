...
class DPAccessory(QWidget):
    ...
    item_restore = Signal(str, int, name='item_restore')
    ...
    def setup_accessory(self, acc_act, pos_x, pos_y):
        ...
        if acc_act.get('name','') == 'item_drop':
            ...
            if acc_act['pet_name'] in self.subpet_dict.keys():
                # Same mini-pet already exists: keep single-instance, ignore duplicate summon.
                exist_idx = self.subpet_dict.get(acc_act['pet_name'])
                if exist_idx in self.acc_dict:
                    return
                # stale mapping cleanup
                self.subpet_dict.pop(acc_act['pet_name'], None)
            if acc_act['pet_name'] in self.subpet_dict.keys():
                return
            # Call the mini-pet
            self.acc_dict[acc_index] = SubPet(acc_index, acc_act['pet_name'],
        ...
        self.acc_dict[acc_index].closed_acc.connect(self.remove_accessory)
        if hasattr(self.acc_dict[acc_index], 'item_restore'):
            self.acc_dict[acc_index].item_restore.connect(self.item_restore)
...
class QItemDrop(QWidget):
    ...
    def __init__(self, acc_index,
        ...
        self.gravity = 1.0
        self.waitn = 0
        self.timeout_ms = max(400, int(self.acc_act.get('timeout_ms', 3000)))
        self.draggable = bool(self.acc_act.get('draggable', True))
        self.return_on_close = bool(self.acc_act.get('return_on_close', False))
        self.return_item_name = self.acc_act.get('return_item_name', None)
        self.return_item_count = int(self.acc_act.get('return_item_count', 1))
        self.is_follow_mouse = False
        self.mouse_drag_pos = self.pos()
        self.mouse_track = []
        self.timer = QTimer()
    ...
    def closeEvent(self, event):
        if self.return_on_close and self.return_item_name:
            self.item_restore.emit(self.return_item_name, max(1, self.return_item_count))
        # we don't need the notification anymore, delete it!
        self.closed_acc.emit(self.acc_index)
        self.deleteLater()
    ...
    def Action(self):
        ...
        if self.finished:
            self.waitn += 1
            if self.waitn >= self.timeout_ms/20:
                self.timer.stop()
                self._closeit()
                return
            else:
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.waitn = 0
            self.mouse_drag_pos = event.globalPos() - self.pos()
            self.mouse_track = [event.globalPos()]
            event.accept()
    ...
    def mouseMoveEvent(self, event):
        ...
        if Qt.LeftButton and self.is_follow_mouse:
            self.move(event.globalPos() - self.mouse_drag_pos)
            self.mouse_track.append(event.globalPos())
            if len(self.mouse_track) > 5:
                self.mouse_track.pop(0)
            event.accept()

    def mouseReleaseEvent(self, event):
        if not self.draggable:
            return
        if event.button() == Qt.LeftButton and self.is_follow_mouse:
            self.is_follow_mouse = False
            if len(self.mouse_track) >= 2:
                p0 = self.mouse_track[-2]
                p1 = self.mouse_track[-1]
                self.v_x = max(-12, min(12, (p1.x() - p0.x()) / 2))
                self.v_y = max(-14, min(14, (p1.y() - p0.y()) / 2))
            self.mouse_track = []
            event.accept()
