...
class DPNote(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        sys_note_conf = dict(json.load(open(os.path.join(basedir, 'res/icons/note_icon.json'), 'r', encoding='utf-8-sig')))
        try:
            pet_note_conf = dict(json.load(open(os.path.join(basedir, 'res/role/{}/note/note.json'.format(settings.petname)), 'r', encoding='utf-8-sig')))
        except:
            pet_note_conf = {}
        self.icon_dict, self.sound_dict = self.init_note(sys_note_conf, pet_note_conf)
        pet_cof = dict(json.load(open(os.path.join(basedir, 'res/role/{}/pet_conf.json'.format(settings.petname)), 'r', encoding='utf-8-sig')))
    ...
    def setup_bubbleText(self, bubble_dict, pos_x, pos_y):
        ...
        side_mode = bubble_dict.get('_side_mode', None)
        ...
        # Limit on-screen bubbles to at most 3.
        if len(self.bubble_dict) >= 3:
            oldest_note_index = next(iter(self.bubble_dict.keys()))
            try:
                self.bubble_dict[oldest_note_index]._closeit()
            except Exception:
                pass
...
class BubbleText(QFrame):
    ...
    def __init__(self, note_index,
        ...
        self.side_mode = side_mode if side_mode in ('left', 'right') else None
        self.pos_x = pos_x
    ...
    def _reposition(self):
        if self.side_mode in ('left', 'right') and settings.pet_hitbox:
            hitbox_x, hitbox_y, hitbox_w, hitbox_h = settings.pet_hitbox
            side_padding = 12
            if self.side_mode == 'left':
                target_x = int(hitbox_x + hitbox_w + side_padding)
            else:
                target_x = int(hitbox_x - self.width() - side_padding)
            target_y = int(hitbox_y + max(6, int(0.28 * hitbox_h)) - self.height_margin)
        else:
            target_x = int(self.pos_x - self.width() // 2)
            target_y = int(self.pos_y - self.height() - self.height_margin)

        try:
            if settings.current_screen is not None:
                geo = settings.current_screen.availableGeometry()
                min_x = geo.left() + 4
                max_x = geo.right() - self.width() - 4
                min_y = geo.top() + 4
                max_y = geo.bottom() - self.height() - 4
                if max_x >= min_x:
