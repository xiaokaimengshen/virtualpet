...
class StatusCard(SimpleCardWidget):
    ...
    def __init_Card(self):
        ...
        if os.path.exists(info_file):
            info = json.load(open(info_file, 'r', encoding='utf-8-sig'))
...
class itemTabWidget(QWidget):
    ...
    item_drop = Signal(object, name='item_drop')
    ...
    def __init__(self, items_data, item_types, sizeHintDyber, tab_index, parent=None):
        ...
        self.externalDragging = False
        self.dragPreview = None
        self.dragStartGlobal = QPoint()
    ...
    def add_item(self, item_name, n_items, emit_anim=True, emit_note=True):
    ...
    def autofeed(self):
        # Auto-feed should pick any usable consumable in backpack,
        # not only the first slot.
        for idx in sorted(self.cells_dict.keys()):
            cell = self.cells_dict[idx]
            if cell.item_type != 'consumable':
                continue
            item_name = cell.item_name
            if not self._can_use_consumable(item_name):
                continue
    ...
    def _can_use_consumable(self, item_name):
        item_conf = self.items_data.item_dict[item_name]

        # Item adds HP, but HP is already full and has no FV effect.
        if (settings.pet_data.hp == (settings.HP_TIERS[-1] * settings.HP_INTERVAL) and
                item_conf['effect_HP'] >= 0):
            if item_conf['effect_FV'] == 0:
                return False
            # FV already full
            elif ((settings.pet_data.fv_lvl == (len(settings.LVL_BAR) - 1)) and
                  (settings.pet_data.fv == settings.LVL_BAR[settings.pet_data.fv_lvl]) and
                  item_conf['effect_FV'] > 0):
                return False

        # HP cost > current HP
        if (settings.pet_data.hp + item_conf['effect_HP']) < 0:
            return False

        # Character-specific check
        pet_limit = item_conf.get('pet_limit', [])
        if pet_limit and settings.petname not in pet_limit:
            self.item_note.emit(
                'system',
                f"[{item_name}] {self.tr('仅能在切换至')} [{'、'.join(pet_limit)}] {self.tr('后使用哦')}"
            )
            return False

        return True

    def _is_drop_on_pet(self, global_pos: QPoint):
        hitbox = getattr(settings, 'pet_hitbox', None)
        if not hitbox:
            return False
        x, y, w, h = hitbox
        return (x <= global_pos.x() <= (x + w)) and (y <= global_pos.y() <= (y + h))

    def _start_external_drag(self, item_widget: PetItemWidget, global_pos: QPoint):
        if self.externalDragging:
            return

        self.externalDragging = True
        self.isDraging = False

        self.dragPreview = QLabel(None)
        self.dragPreview.setWindowFlags(Qt.FramelessWindowHint | Qt.ToolTip | Qt.WindowStaysOnTopHint)
        self.dragPreview.setAttribute(Qt.WA_TranslucentBackground, True)
        pix = item_widget.image.scaled(ITEM_SIZE, ITEM_SIZE, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.dragPreview.setPixmap(pix)
        self.dragPreview.setFixedSize(ITEM_SIZE, ITEM_SIZE)
        self.dragPreview.show()
        self._update_external_drag_preview(global_pos)
        self.grabMouse()

    def _update_external_drag_preview(self, global_pos: QPoint):
        if not self.dragPreview:
            return
        self.dragPreview.move(global_pos.x() - ITEM_SIZE // 2, global_pos.y() - ITEM_SIZE // 2)

    def _stop_external_drag(self):
        if self.dragPreview:
            self.dragPreview.hide()
            self.dragPreview.deleteLater()
            self.dragPreview = None
        if self.externalDragging:
            self.releaseMouse()
        self.externalDragging = False

    def _external_consume_food(self, item_name, feed_to_pet):
        settings.pet_data.change_item(item_name, item_change=-1)
        self.item_num_changed.emit(item_name)

        if self.pressed_cell in self.cells_dict:
            self.cells_dict[self.pressed_cell].consumeItem()
        ...
        if feed_to_pet:
            self.use_item_inven.emit(item_name)
            if self.items_data.item_dict[item_name]['buff']:
                self.addBuff.emit(item_name)
        else:
            self.item_drop.emit({
                "item_name": item_name,
                "drop_type": "external_food",
                "draggable": True,
                "timeout_ms": 3000,
                "return_on_close": True
            })

    def _handle_external_drop(self, global_pos: QPoint):
        if self.pressed_cell is None or self.pressed_cell not in self.cells_dict:
            return

        item = self.cells_dict[self.pressed_cell]
        if item.item_type != 'consumable' or item.item_name == 'None':
            return
        ...
        feed_to_pet = self._is_drop_on_pet(global_pos)
        self._external_consume_food(item_name, feed_to_pet)
    ...
    def mousePressEvent(self, e: QMouseEvent):
        ...
        self.dragPos = e.pos()
        self.dragStartGlobal = e.globalPos()
    ...
    def mouseMoveEvent(self, e: QMouseEvent):
        ...
        if self.externalDragging:
            self._update_external_drag_preview(e.globalPos())
            return

        pressed_item = self.cells_dict[self.pressed_cell]

        # Drag food out of backpack => release on pet to feed, otherwise drop as falling item.
        if pressed_item.item_type == 'consumable':
            moved = (e.globalPos() - self.dragStartGlobal).manhattanLength()
            if moved >= 8 and not self.rect().contains(e.pos()):
                self._start_external_drag(pressed_item, e.globalPos())
                return
    ...
    def mouseReleaseEvent(self, e):
        ...
        if self.externalDragging:
            ...
            self._stop_external_drag()
            self._handle_external_drop(global_pos)
            self.nearest_index = None
            self.pressed_cell = None
            return
