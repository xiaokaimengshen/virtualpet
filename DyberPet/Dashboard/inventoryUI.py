...
If there is any item in the first cell of the consumable item tab, this item will be automatically fed to the pet when the HP is lower than 60.""")
    ...
    def addCoins(self, value, note=True, anim=True, note_msg=''):
        ...
        if value > 0:
            value = int(round(value * settings.COIN_GAIN_MULTIPLIER))
    ...
    def add_item(self, item_name, n_items, emit_anim=True, emit_note=True):
        ...
        widget.add_item(item_name, n_items, emit_anim=emit_anim, emit_note=emit_note)

    def restore_item(self, item_name, n_items=1):
        self.add_item(item_name, n_items, emit_anim=False, emit_note=False)
